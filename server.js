...
const {
  ULTRAMSG_INSTANCE_ID,   // UltraMsg instance ID (e.g. instance12345)
  ULTRAMSG_TOKEN,         // UltraMsg token from dashboard
  META_VERIFY_TOKEN,      // kept for health-check endpoint
  SHOPIFY_STORE_URL,      // chefmayshop.myshopify.com
...
function msgRetry(orderName) {
  return (
    ...
    `*1* - تأكيد الطلب ✅\n` +
    `*2* - إلغاء الطلب ❌\n\n` +
    `إذا لم نتلقَ ردك سيُوقَف الطلب تلقائياً.`
  );
}
...
// ─── Health check for webhook verify token ────────────────────────────────────
...
// ─── UltraMsg Webhook: Incoming Message (POST) ───────────────────────────────
